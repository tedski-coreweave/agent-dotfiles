...
description: Writing documents for agents. Use when creating or editing a skill, an AGENTS.md/CLAUDE.md, a glossary, or any document an agent consumes through a pointer.
---

Reference for writing any document an agent consumes: a skill, an `AGENTS.md` / `CLAUDE.md`, a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable, since the agent takes the same _process_ every run rather than producing the same output.

When the document you're writing is a skill, also read the "Skill mechanics under pi" section at the end.
...
- **By invocation**, skill-specific: see "Skill mechanics under pi" below.
...
## Skill mechanics under pi

Adapted from upstream's `SKILL-MECHANICS.md`, written for Claude Code.
Pi's mechanics differ in detail but the invocation tradeoff carries
over intact (verified against pi's docs/skills.md):

- Pi has both tiers. A discovered skill's name + description loads
  into the system prompt: context load, in exchange for the agent
  reaching it autonomously. `disable-model-invocation: true` hides the
  skill from the system prompt entirely; only a human typing
  `/skill:name` reaches it: zero context load, full cognitive load.
  Pay context load only when the agent must reach the skill on its
  own.
- A skill too niche to earn its description line has two homes: the
  hidden tier, or a plain file reached by a pointer from whatever
  needs it.
- Shared reference needed by several skills lives in one model-invoked
  skill the others name, or in a plain file they all point at. Hidden
  skills make poor shared reference: nothing advertises them to other
  documents.
- Router skills cure human memory, not agent memory: the agent already
  sees every model-invoked description. A router earns its place only
  once hidden skills multiply past what the human remembers.
